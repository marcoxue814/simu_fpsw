...
for(q in 1:Q)
{
  ...
  npfcbps=npFCBPS.Functional(treat, conf)
  ...
  simufit2_np[[q]]$Covw=Covw
  simufit2_np[[q]]$Covu=Covu
  
  simufit2_np[[q]]$beta.hatw=beta.hatw
...
for(q in 1:Q)
{
  ...
  absCovw[q]=max(abs(simufit2_np[[q]]$Covw))
  
  absCovu[q]=max(abs(simufit2_np[[q]]$Covu))
  
  ISE[q]=mean((simufit2_np[[q]]$beta.hatw-beta)^2)
  
}

sapply(simufit2_np, function(x) nrow(x$Covw)) # L=4
...
boxplot(absCov ~ grp.weight, data=absCov.cb, horizontal = TRUE, 
        main='Nonarametric Covariate Balancing')
