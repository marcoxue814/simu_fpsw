                               ...
                               rho = 0.01, ...)
{
  ...
  K=ntreat + nconf + ntreat*nconf + ntreat^2
  ...
  hfun_given_theta=function(theta)
  {
    ...
    {
      outer(treat.star[i,],treat.star[i,])-diag(ntreat)
    })))
  }
...
for(q in 1:Q)
{
  ...
  zz=cov.wt(xy,wt=c(fcbps$w),cor=TRUE, center=FALSE)
  ...
  simu_np.1[[q]]$Covw=Covw
  simu_np.1[[q]]$Covu=Covu
  
  simu_np.1[[q]]$beta.hatw=beta.hatw
  simu_np.1[[q]]$beta.hatu=beta.hatu
}


save(simu_np.1, file = 'simu_np_1.rda')